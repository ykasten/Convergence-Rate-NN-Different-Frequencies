...
def optimize(cfg, device, net, datasets, pr=False):
    ...
    cfg['n_train'] = trainset.x.shape[0]
    # create optimizer
    ...
    with tr.no_grad():
        ...
        target=valset.y.to(device)
        target = target.type(tr.LongTensor)
        target=target.to(device)
        val_loss_before_training = criterion(val_output_before_training, target.squeeze())
        target = trainset.y.to(device)
        target = target.type(tr.LongTensor)
        target = target.to(device)

        train_output_before_training = net(trainset.x.to(device))
        train_loss_before_training = criterion(train_output_before_training,target.squeeze())
    ...
    while True:
        ...
        target = target.type(tr.LongTensor)
        target = target.to(device)
        val_loss[epoch] = criterion(val_output, target.squeeze())
        target = trainset.y.to(device)
        target = target.type(tr.LongTensor)
        target = target.to(device)

        train_output = net(trainset.x.to(device))
        train_loss[epoch] = criterion(train_output, target.squeeze())
        ...
        if train_loss[epoch-1]<0.05:#train_loss[epoch - 1] / cfg['zero_val_loss'] < cfg['stop_threshold_percent'] / 100:
    ...
    with tr.no_grad():
        ...
        target = target.type(tr.LongTensor)
        target = target.to(device)
        val_loss_after_training = criterion(val_output_after_training, target.squeeze())
        target = trainset.y.to(device)
        target = target.type(tr.LongTensor)
        target = target.to(device)

        train_output_after_training = net(trainset.x.to(device))
        train_loss_after_training = criterion(train_output_after_training, target.squeeze())
