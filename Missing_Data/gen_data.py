...
def gen_xy(cfg, type):
    ...
    elif cfg['gen_x_func'] == 'gen_x_circle_holes_':
        x, theta = gen_x_circle_holes_(n)
    elif cfg['gen_x_func'] == 'gen_x_circle_pos':
        x, theta = gen_x_circle_pos(n, cfg['ks'][0])
...
def gen_x_circle_holes(n):
    theta0 = np.linspace(-np.pi, -np.pi/3, n // 2 + 1)
    theta0 = theta0[:-1]
    theta1 = np.linspace(np.pi/3, np.pi, n - n // 2 + 1)
    theta1 = theta1[:-1]
    theta = np.concatenate((theta0, theta1), 0)
    x = np.stack([np.cos(theta), np.sin(theta)]).T
    return x, theta

def gen_x_circle_holes_(n):
    theta0 = -np.pi + 2 * np.pi * np.linspace(0, 7/24, n // 2 )
    theta1 = np.pi - 2 * np.pi * np.linspace(0, 7/24, n - n // 2 )
    theta = np.concatenate((theta0, theta1), 0)
    x = np.stack([np.cos(theta), np.sin(theta)]).T
    return x, theta


def gen_x_circle_pos(n, k):
    theta = np.empty(n)
    m = 0
    for q in range(k):
        nq = int(n / k)
        if q == k-1:
            nq = n - (k-1)*nq
        thetaq = 2*np.pi*(q / k + np.linspace(0, 1, nq) / (2*k)) - np.pi
        theta[m:m+nq] = thetaq
        m += nq
    x = np.stack([np.cos(theta), np.sin(theta)]).T
    return x, theta
